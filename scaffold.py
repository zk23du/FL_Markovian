...
#                 param.copy_(torch.stack([client_model.state_dict()[name] for client_model in client_models], 0).mean(0))

#         # Update global control variates
#         with torch.no_grad():
#             for name, param in global_model.named_parameters():
#                 c_global[name] = torch.stack([c_clients[client_index][name] + (client_model.state_dict()[name] - param) / (n_local_steps * lr)
#                                               for client_index, client_model in enumerate(client_models)], 0).mean(0)
    ...
    for global_step, client_data in enumerate(zip(*client_dataloaders)):
        ...
        with torch.no_grad():
            ...
            for client_index in range(n_clients):
                client_test_dataloader = client_test_dataloaders[client_index]
                for test_data, test_target in client_test_dataloader:
                    test_loss += criterion(global_model(test_data[0]), test_target).item()
            test_loss /= (len(client_test_dataloaders) * len(client_test_dataloaders[0]))
            writer.log({
                "test_loss": test_loss,
                "communication_round": global_step
